...
import {
  ...
  Phone, Mail, MessageCircle, Star, Store,
} from 'lucide-react';
import { repsStore, shopsStore } from '@/lib/store';
import { formatCurrency } from '@/lib/dummy-data';
import type { SalesRep, Shop } from '@/types';
...
function RepCard({ rep, shopId, onRemove, onToggleAlerts }: {
  rep: SalesRep; shopId: string; onRemove: () => void; onToggleAlerts: () => void;
}) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-primary-100 shadow-card p-4">
      ...
      <div className="space-y-1.5 mb-3">
        ...
        {rep.whatsappNumber && (
          <a href={`https://wa.me/${rep.whatsappNumber.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer"
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {shopId ? `${active} active · ${pending} pending` : 'Manage reps across your shops'}
          </p>
        </div>
        {shopId && (
          <Button variant="primary" size="md" onClick={() => setShowInvite(true)}>
            <UserPlus className="w-4 h-4 mr-1.5" />Invite Rep
          </Button>
        )}
      </div>
      ...
      {!shopId && (
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <p className="text-gray-500">Create a shop to start adding sales reps.</p>
        </div>
      )}
      ...
      {shopId && (
        <>
          {/* How it works */}
          {!repsLoading && (!reps || reps.length === 0) && (
          ...
          {repsLoading && <div className="flex justify-center py-12"><Spinner size="lg" /></div>}
          ...
          {!repsLoading && (!reps || reps.length === 0) && (
            <div className="flex flex-col items-center py-12 text-center">
              ...
              <p className="text-sm text-gray-400 mb-5 max-w-xs">
                Invite your first sales rep to help manage orders and receive alerts.
              </p>
              <Button variant="primary" size="md" onClick={() => setShowInvite(true)}>
                <UserPlus className="w-4 h-4 mr-1.5" />Invite First Rep
              </Button>
            </div>
          )}
      ...
      <Modal isOpen={showInvite} onClose={() => setShowInvite(false)} title="Invite Sales Rep">
        {shopId && <InviteRepForm shopId={shopId} onClose={() => setShowInvite(false)} />}
      </Modal>
    </div>
  );
}
