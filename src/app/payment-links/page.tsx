...
import {
  Link as LinkIcon, Plus, Copy, CheckCircle, Trash2, ExternalLink,
  ToggleLeft, ToggleRight, Store,
} from 'lucide-react';
import { paymentLinksStore, shopsStore } from '@/lib/store';
import { formatCurrency, SHORT_STOREFRONT_URL } from '@/lib/dummy-data';
import type { PaymentLink, Shop } from '@/types';
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <textarea rows={2}
          ...
          placeholder="What is this payment for?" value={form.description}
          onChange={(e) => set('description', e.target.value)} />
      </div>
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => set('allowCustomAmount', !form.allowCustomAmount)}
          className={`w-10 h-6 rounded-full transition-colors ${form.allowCustomAmount ? 'bg-primary' : 'bg-gray-200'}`}>
      ...
      {!form.allowCustomAmount && (
        <Input label="Amount (NGN) *" type="number" min="1" value={form.amount}
          onChange={(e) => set('amount', e.target.value)} placeholder="5000" />
      )}
      <div>
        ...
        <div className="flex items-center rounded-xl border border-primary-200 overflow-hidden">
          ...
          <input className="flex-1 px-3 py-2.5 text-sm bg-white outline-none" placeholder="auto-generated"
            value={form.slug} onChange={(e) => set('slug', e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <Input label="Max Uses (optional)" type="number" min="1" value={form.maxUses}
          onChange={(e) => set('maxUses', e.target.value)} placeholder="Unlimited" />
        <Input label="Expires At (optional)" type="date" value={form.expiresAt}
      ...
      <div className="flex gap-3">
        ...
        <Button variant="primary" size="md" onClick={handleSave} loading={saving} className="flex-1">
          <LinkIcon className="w-4 h-4 mr-1.5" />Create Link
        </Button>
      </div>
    </div>
  );
...
export default function PaymentLinksPage() {
  ...
  const { data: links, isLoading: linksLoading } = useQuery<PaymentLink[]>({
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Create shareable links to collect payments instantly</p>
        </div>
        {shopId && (
      ...
      {!shopId && (
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <p className="text-gray-500">Create a shop to start generating payment links.</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mb-6">
            <p className="font-semibold text-blue-800 text-sm mb-1 flex items-center gap-2">
              <LinkIcon className="w-4 h-4" />How payment links work
            </p>
          ...
          {linksLoading && <div className="flex justify-center py-12"><Spinner size="lg" /></div>}
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Create Payment Link">
        {shopId && <CreateLinkForm shopId={shopId} onClose={() => setShowCreate(false)} />}
      </Modal>
    </div>
  );
}
